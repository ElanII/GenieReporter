...
<html>
<head>
	@include('newpartials/head',['title'=>"DEXA 12"])
</head>
<body>
	{{-- Sidebar --}}
	...
	<?php
		...
		$yearsAgo = date("Y-m-d H:i:s",strtotime('-12 month'));


		// BHMC ITEM COMPLETE.
		$patientsHaveDexa = DB::connection('mysql')->select("SELECT DISTINCT patientID FROM bhmcSales WHERE ITEMNUM IN ('12312','12321') AND SERVICEDATE >= :yearsAgo",
			['yearsAgo'=>$yearsAgo]);
		...
		$patientsHaveDexa = DB::connection('mysql')->select("SELECT DISTINCT patientID FROM bhmcSales WHERE ITEMNUM IN ('12312','12321')");
		foreach ($patientsHaveDexa as $value) {
		...
		$sql0 = "SELECT DISTINCT PT_Id_Fk FROM Sale WHERE ItemNum IN ('12312','12321') AND ServiceDate >= :yearsAgo";
		try {
		...
		$sql4 = "SELECT DISTINCT PT_Id_Fk FROM Sale WHERE ItemNum IN ('12312','12321')";
		try {
	...
	{{-- Table Section --}}
	@include('newpartials/saleTable',['title'=>"DEXA 12 ( '12312' , '12321' )  Has had before but not done in 12 months"])
	{{-- Footer --}}
