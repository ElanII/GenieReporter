...
<body>
	...
	<?php
		...
		$sql1 = "SELECT DISTINCT Id,FirstName,Surname,HomePhone,MobilePhone,LastSeenDate,DOB,ChartOrNHS,Age,Inactive FROM Patient WHERE
		 Id IN (".implode(',', $PTID0).") AND Id NOT IN (".implode(',', $PTID2).") AND Age > 20 AND Age < 55 AND SmokingFreq IN (1,3,2) AND Postcode LIKE '2880'";
		try {
	...
	{{-- Table Section --}}
	@include('newpartials/table',['title'=>"Search Results for Smokers needing CP..."])
	<?php 
